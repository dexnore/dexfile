...
func buildPlatformOpt(opt *dexfile.ConvertOpt) *platformOpt {
	buildPlatforms := opt.Config.BuildPlatforms
	targetPlatform := opt.TargetPlatform
	implicitTargetPlatform := false

	if opt.TargetPlatform != nil && opt.Config.BuildPlatforms == nil {
