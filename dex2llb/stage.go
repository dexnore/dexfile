...
func solveStage(ctx context.Context, target *dispatchState, buildContext *mutableDexfileOutput, opt dispatchOpt, copts ...llb.ConstraintsOpt) (_ *dispatchState, breakCmd bool, err error) {
	...
	ds, ctxPaths, breakCmd, err := solveDispatchableStages(ctx, target, opt, copts...)
	if err != nil {
...
func solveDispatchableStages(ctx context.Context, target *dispatchState, opt dispatchOpt, copts ...llb.ConstraintsOpt) (_ *dispatchState, _ map[string]struct{}, breakCmd bool, err error) {
	...
	for _, d := range opt.allDispatchStates.states {
		...
		if d.image.Config.WorkingDir != "" {
			if err = dispatchWorkdir(d, &converter.WorkdirCommand{Path: d.image.Config.WorkingDir}, false, nil); err != nil {
				return nil, ctxPaths, false, parser.WithLocation(err, d.Location())
			}
		}
		...
		for _, cmd := range d.commands {
			if breakCmd, err = dispatch(ctx, d, cmd, opt, copts...); err != nil {
				...
				return d, ctxPaths, breakCmd, err
			}
			if breakCmd {
				return d, ctxPaths, true, nil
			}
		}
	...
	return target, ctxPaths, false, nil
}
