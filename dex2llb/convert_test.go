...
RUN ls -l
`
	_, _, _, _, err := Dexfile2LLB(appcontext.Context(), []byte(df), dexfile.ConvertOpt{})
...
RUN ls -l
`
	state, _, _, _, err := Dexfile2LLB(appcontext.Context(), []byte(df), dexfile.ConvertOpt{})
	require.NoError(t, err)
...
RUN echo bar
`
	_, _, baseImg, _, err := Dexfile2LLB(appcontext.Context(), []byte(df), dexfile.ConvertOpt{})
	require.NoError(t, err)
