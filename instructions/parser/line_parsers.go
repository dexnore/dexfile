...
// Node object which contains the whole statement. Dexfiles have varied
...
var (
	errDexfileNotStringArray = errors.New("when using JSON array syntax, arrays must be comprised of strings only")
	errDexfileNotJSONArray   = errors.New("not a JSON array")
)
...
func parseJSON(rest string) (*Node, map[string]bool, error) {
	...
	if !strings.HasPrefix(rest, "[") {
		return nil, nil, errDexfileNotJSONArray
	}
	...
	for _, str := range myJSON {
		s, ok := str.(string)
		if !ok {
			return nil, nil, errDexfileNotStringArray
		}
...
func parseMaybeJSON(rest string, d *directives) (*Node, map[string]bool, error) {
	...
	if errors.Is(err, errDexfileNotStringArray) {
		return nil, nil, err
	}
