...
// Command interface is implemented by every possible command in a Dexfile.
//
...
// withNameAndCode is the base of every command in a Dexfile (String() returns its source code)
...
type Stage struct {
	...
	Loc        []parser.Range // location of the defining FROM command
}
...
func (s *Stage) Location() []parser.Range {
	return s.Loc
}
...
func IsCurrentStage(s []Stage, name string) bool {
	...
	return s[len(s)-1].StageName == name
}

// CurrentStage returns the last stage from a list of stages.
func CurrentStage(s []Adder) (Adder, error) {
	if len(s) == 0 {
...
func HasStage(s []Stage, name string) (int, bool) {
	for i, stage := range s {
		...
		if strings.EqualFold(stage.StageName, name) {
			return i, true
		}
	}
	return -1, false
